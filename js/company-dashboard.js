  ...
  static async fetchCompanyJobs(companyId) {
    try {
      const response = await fetch(
        `${API_BASE_URL}/companies/${companyId}/jobs`
      );
      const data = await response.json();
        ...
        <div class="job-card-dashboard" data-job-id="${job.id}">
            <div class="job-card-header">
                ...
                <div class="job-actions">
                    <button class="btn-secondary edit-job-btn" data-job-id="${
                      job.id
                    }">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                    <button class="btn-secondary view-applications-btn" data-job-id="${
                      job.id
                    }">
                        <i class="fas fa-users"></i> Applications
                    </button>
                    <button class="btn-danger delete-job-btn" data-job-id="${
                      job.id
                    }">
  ...
  return `
        <div class="application-card" data-application-id="${application.id}">
            ...
            <div class="application-actions">
                <button class="btn-secondary view-application-btn" data-application-id="${
                  application.id
                }">
                    View Details
                </button>
                <select class="status-select" data-application-id="${
                  application.id
                }">
  ...
  try {
    ...
    if (currentJobs.length === 0) {
      ...
      jobsContainer.innerHTML = `
                <div class="jobs-grid">
                    ${currentJobs
                      .map((job) => createJobCardTemplate(job))
                      .join("")}
                </div>
            `;
...
async function loadApplications() {
  ...
  try {
    ...
    if (currentApplications.length === 0) {
      ...
      applicationsContainer.innerHTML = currentApplications
        .map((app) => createApplicationCardTemplate(app))
        .join("");
...
async function loadRecentApplications() {
  const recentApplicationsContainer =
...
function showApplicationModal(application) {
  ...
  details.innerHTML = `
        <div class="application-detail">
            ...
            <p><strong>Applied on:</strong> ${formatDate(
              application.createdAt
            )}</p>
            ${
              application.coverLetter
                ? `<div><strong>Cover Letter:</strong><br><p>${application.coverLetter}</p></div>`
                : ""
            }
            ${
              application.resumeUrl
                ? `<div><strong>Resume:</strong><br><a href="${application.resumeUrl}" target="_blank" class="btn-primary">View Resume</a></div>`
                : ""
            }
        </div>
    `;
...
function filterApplicationsByJob(jobId) {
  ...
  applicationsContainer.innerHTML = filteredApps
    .map((app) => createApplicationCardTemplate(app))
    .join("");
  addApplicationEventListeners();
}
...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("click", hideApplicationModal);
  document
    .getElementById("cancelJobForm")
    .addEventListener("click", hideJobModal);
